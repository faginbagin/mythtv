...
// but we need to call it on every field
    ...
    if(CONFIG_MPEG_XVMC_VLD_DECODER && s->avctx->xvmc_vld_hwslice)
                             ...
                             const uint8_t **buf, int buf_size)
{
    ...
    for(;;) {
        ...
        if (CONFIG_MPEG_DVDV_DECODER && s->avctx->dvdv)
            DVDV_init_block(s);
...
#if CONFIG_MPEG_XVMC_VLD_DECODER
